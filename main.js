    ...
    array.map(e=>
        e.filter(e=>{
            ...
            $tile.addEventListener('mousedown',()=> {
                console.log(tile.getTileOption('id'))
                changeTilesColor($tile)
            } )
    ...
    moveTile(){
        ...
        if (this.color == 'white'){
            ...
            return new Promise((res, rej)=>{
                this.color='yellow'
                res(this.id, this.color)

            }).then(res=>{
                freeTiles.forEach(e=>{
                    document.querySelector(`.tile[id='${this.id-e}']`).classList.add('tile-green')
                })
            })
        }
...
const checkerWhite = new Checker({
    color: 'white',
    type: 'checker',
    id: 64,
})

const ladyWhite = new Lady({
    color: 'white',
    type: 'lady',
    id: 55,
})
